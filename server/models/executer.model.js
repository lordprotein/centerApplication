...
Executer.read = (req, result) => {
    ...
    db.query('SELECT executers.`Full name`, roles.role FROM executers INNER JOIN roles WHERE executers.ID_ROLE = roles.ID AND executers.ID = ?', id, (err, res) => {
...
Executer.create = (req, result) => {
    ...
    db.query('INSERT INTO applications_of_executers (ID, ID_EXECUTER, ID_APPLICATION) VALUES (?, ?, ?)', [id, ID_EXECUTER, ID_APPLICATION], (err, res) => {
        ...
        db.query('UPDATE applications SET Status = \'Process\' WHERE ID = ?', ID_APPLICATION, (err, res) => {
            if (err) {
